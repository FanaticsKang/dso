...
PhotometricUndistorter::PhotometricUndistorter(std::string file,
...

  {
    ...
    for (int i = 0; i < GDepth; i++)
      G[i] =
          255.0 * (G[i] - min) / (max - min);  // make it to 0..255 => 0..255.
  }
...
void Undistort::readFromFile(const char* configFileName, int nPars,
  ...
  if (parsOrg[2] < 1 && parsOrg[3] < 1) {
    printf(
        ...
        "and will rescale this by image width / height to fx=%f, fy=%f, cx=%f, "
        "cy=%f.\n\n",
    ...
    // pixel at (0,0)
    // contains the integral over intensity over [0,0]-[1,1], whereas I assume
    // the pixel (0,0)
    // to contain a sample of the intensity ot [0,0], which is best approximated
